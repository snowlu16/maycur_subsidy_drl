...
11. **Drools `dialect "java"` 语法编译防错避坑指南**：
    ...
    - **直接传参并补齐三项日期属性（无需格式转换）**：根据官方接口规范，`consumeDate` 类型为 `org.joda.time.DateTime`，**直接取 `$submittedAt / DateTime.now()` 即可，不要进行任何自定义格式转换或调抹零方法**！同时为确保前端表格读取消费日期时绝不显示为空，需在 `insert` 前同步将 `date`、`startDate` 和 `endDate` 三个属性均设为该日期：
      ```java
