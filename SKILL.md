...
> 
> **✍️ AI 智能生成规范提示词模板 (Prompt Templates)**：
> **最新收录** 👉 **[生成每刻补贴规则脚本：标准提问提示词手册 (standard_prompts.md)](file:///references/standard_prompts.md)**！内含 1 分钟快速填空版 Prompt、高阶 XML 定制 Prompt 以及四大实战场景 Copy-Paste 提问指令卡，确保 AI 自动生成的代码一次性高成功率！
> 
> **📚 45 个全量实操完整案例库**：
> 本技能库 `examples/` 目录下已全量内置 **45 个真实生产环境完整 `.md` 脚本**（含所有 `1.1`~`1.13` 及 `2.1`~`2.19` 复杂场景）。所有案例均可在 👉 **[每刻官方场景与完整源码速查索引](file:///references/usg5m5_templates.md)** 中一键点击跳转查看！
...
10. **纯报销单/自定义表单补贴的“触发死锁防坑”（无行程+无费用明细不会进入执行规则）**：
    当用户在报销单 (`Reimburse`) 上直接使用自定义表单字段（如自定义天数 CF105）计算全单补贴，且该单据**既没有行程信息 (`TravelRoute` 为空)，也没有任何费用明细 (`Expense` 为空/数量为 0)** 时，**每刻底层的费控补贴规则引擎会直接跳过，根本不会触发进入 `.drl` 规则执行！（即触发失败）**
    因此在调试或测试无行程纯报销单补贴时，**单据上必须至少存在一条费用明细 (`expenses.size() > 0`)**（哪怕是先手填或挂一笔普通的打车/餐饮明细），此时点击计算或测试才能成功进入 DRL 脚本执行 (`表单没有行程，费用明细不为空 -> 成功`)。
11. **Drools `dialect "java"` 语法编译防错避坑指南**：
    在编写 `.drl` 脚本的 `then ... end` 块时，因为使用了 Java 方言，需严格遵守 Java 编译规则，防止控制台抛出 `Unable to build expression for consequence: offset -1, count 1, length 0`：
    - **注释安全**：严禁在单行代码末尾无换行或 `end` 之前紧邻位置写 `//` 单行注释，注释后必须有干净独立的换行 (`\n`)，避免截断 AST 语法树。
    - **构造方法签名对齐**：`new AllowanceResult(...)` 必须严格遵守官方 4 种重载之一（传参类型为 `(DateTime, String, BigDecimal, String, String)` 或含 `consumeLocation/destination` 扩展），严禁自创参数组合。
    - **直接传参并补齐三项日期属性（无需格式转换）**：根据官方接口规范，`consumeDate` 类型为 `org.joda.time.DateTime`，**直接传入原生 `DateTime` 即可，不要进行任何自定义格式转换或调抹零方法**！同时为确保前端表格读取消费日期时绝不显示为空，需在 `insert` 前同步将 `date`、`startDate` 和 `endDate` 三个属性均设为该日期。
12. **严禁使用 `submittedAt` 与 `firstSubmittedAt` 获取消费或补贴日期（草稿与计算运行期陷阱）**：
    当报销单处在填报草稿、点击“测试计算 / 预览补贴金额”阶段，或者未正式提交流转前，单据上的 `$submittedAt` 和 `$firstSubmittedAt` 字段值均为 **`0` (`1970-01-01T08:00:00.000`)**！
    如果脚本写了 `($submittedAt != null) ? $submittedAt : DateTime.now()`，因为 `$submittedAt` 并不是 `null`（而是带时间戳 `0` 的 `DateTime` 对象），会导致生成的补贴消费日期全变成 **1970年**，进而引发前端日期控件解析异常或展示为空！
    因此：**在所有规则脚本中，后续绝对不要再使用 `$submittedAt` 和 `$firstSubmittedAt` 字段！** 对于无行程的报销单全单补贴，请优先从单据现有明细行读取日期（`$reimburse.getExpenses().get(0).getConsumeDate()`），若为空再回退使用 `DateTime.now()`：
    ```java
    ...
    result.setDate(consumeDate); result.setStartDate(consumeDate); result.setEndDate(consumeDate);
    insert(result);
    ```
