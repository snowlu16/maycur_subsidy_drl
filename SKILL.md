...
11. **Drools `dialect "java"` 语法编译防错避坑指南**：
    ...
    - **兼容老旧 JodaTime 且防止前端显示“无 consumeDate”的通用日期归零方案**：严禁直接调用 `.withTimeAtStartOfDay()`（部分线上环境依赖老旧抛 `NoSuchMethodError`）。如需按 `年-月-日` (`yyyy-MM-dd`) 格式剔除时分秒，并确保前端报销表格正确渲染消费日期而不为空，**必须且只能使用通用重载构造当日零点**，并同时补齐属性赋值：
      ```java
