...
11. **Drools `dialect "java"` 语法编译防错避坑指南**：
    ...
    - **严禁使用 `withTimeAtStartOfDay()`**：传 `DateTime consumeDate` 进 `AllowanceResult` 时，**请直接传入 `$submittedAt / DateTime.now()` 即可（去掉 `withTimeAtStartOfDay()`）**！因每刻部分线上集群的 `joda-time` 依赖版本不支持该方法，加上会导致运行时报错 `NoSuchMethodError` 或 `Unable to build expression`！
