...
rule "行程-初始化TravelHalfDayKeys，用来去重"
    ...
    then
		...
		String feeCode="BT";
        insert(new TravelHalfDayKeys(dateKeys,bizCode,feeCode));
end
...
rule "行程参与人-按照半天拆分"
    ...
    when
		...
		TravelHalfDayKeys($dateKeys:dateKeys,$bizCode:bizCode,$feeCode:feeCode);
	...
	then
		...
		if(null!=$reimEmployee.getResidences){
			for(Residence re : $reimEmployee.getResidences){
				String cityOne=re.getPlaceFullCode.split("-")[2].toString;
				if(cityOne.equals(destination)){
