# real_2.11_含报销和同行人_全天_行程汇总_三套标准
...
declare TravelHalfDayKeys
	...
	threeBizCode: String // 第三个标准补贴编码
    threeFeeCode: String // 第三个费用类型编码
end
...
rule "初始化参数"
    ...
    then
        ...
        String secondBizCode = "1234"; // 设置第二个标准补贴编码
        String secondFeeCode = "1234"; // 设置第二个费用类型编码
		String threeBizCode = "jtbt"; // 设置第三个标准补贴编码
        String threeFeeCode = "jtbt"; // 设置第三个费用类型编码
        insert(new TravelHalfDayKeys(new HashMap(), firstBizCode, firstFeeCode, secondBizCode, secondFeeCode, threeBizCode, threeFeeCode));
end
...
rule "报销人和参与人行程费用第一个补贴金额规则"
    ...
    then
        ...
        for(Object obj : dateTimes) {
            ...
            logger.info("补贴报销人行程日期为" + dateTime + ",补贴报销人编号为" + $reimEmployee.getUserCode() + ",补贴标准金额为" + allowanceAmount + ",补贴标准编码为" + $firstBizCode);
            ...
            if ($travelPartnerInfo != null) {
                ...
                for(TravelPartner travelPartner : $travelPartnerInfo.getInternalTravelPartner) {
                    ...
                    logger.info("补贴参与人行程日期为" + dateTime + ",补贴参与人编号为" + travelPartner.getUserCode() + ",补贴标准金额为" + partnerAllowanceAmount + ",补贴标准编码为" + $firstBizCode);
        ...
        if(amount.compareTo(BigDecimal.ZERO) == 1) {
            ...
            /**返回补贴结果 */
            AllowanceResult allowanceResult = new AllowanceResult($startDate, $endDate, $firstFeeCode, amount, $baseCcy, $collectionCcy);
            /**设置消费城市 */
...
rule "报销人和参与人行程费用第二个补贴金额规则"
    ...
    then
        ...
        for(Object obj : dateTimes) {
            ...
            logger.info("补贴报销人行程日期为" + dateTime + ",补贴报销人编号为" + $reimEmployee.getUserCode() + ",补贴标准金额为" + allowanceAmount + ",补贴标准编码为" + $secondBizCode);

            /**判断报销人行程日期是否重叠 */
            if(!isDayOverlap($dateMap, $reimEmployee.getUserCode() + $secondBizCode, dateTime)) {
                amount = amount.add(allowanceAmount);
            }
            ...
            if ($travelPartnerInfo != null) {
                ...
                for(TravelPartner travelPartner : $travelPartnerInfo.getInternalTravelPartner) {
                    ...
                    logger.info("补贴参与人行程日期为" + dateTime + ",补贴参与人编号为" + travelPartner.getUserCode() + ",补贴标准金额为" + partnerAllowanceAmount + ",补贴标准编码为" + $secondBizCode);

                    /**判断日期是否重叠 */
                    if(!isDayOverlap($dateMap, travelPartner.getUserCode() + $secondBizCode, dateTime)) {
                        amount = amount.add(partnerAllowanceAmount);
                    }
                }
        ...
        if(amount.compareTo(BigDecimal.ZERO) == 1) {
            ...
            /**返回补贴结果 */
            AllowanceResult allowanceResult = new AllowanceResult($startDate, $endDate, $secondFeeCode, amount, $baseCcy, $collectionCcy);
            /**设置消费城市 */
            allowanceResult.setConsumeLocation($destination);
...
rule "报销人和参与人行程费用第三个补贴金额规则"
    salience 10
    when
        // 获取变量参数
        TravelHalfDayKeys(
            $dateMap:dateMap, // 获取日期集合
            $threeBizCode:threeBizCode, // 获取准补贴编码
            $threeFeeCode:threeFeeCode // 获取费用类型编码
        );
        // 获取单据
        Reimburse(
            $travelRoutes:travelRoutes, // 行程信息
        ...
        // 获取补贴信息
		$allowanceStandardInfo : AllowanceStandardInfo(
			$threeBizCode.equals(ruleBizCode) // 补贴标准编码
		) from $allowanceStandardInfos;
    then
        /**设置补贴总金额 */
		BigDecimal amount = BigDecimal.ZERO;
        ...
        for(Object obj : dateTimes) {
            ...
            logger.info("补贴报销人行程日期为" + dateTime + ",补贴报销人编号为" + $reimEmployee.getUserCode() + ",补贴标准金额为" + allowanceAmount + ",补贴标准编码为" + $threeBizCode);

            /**判断报销人行程日期是否重叠 */
            if(!isDayOverlap($dateMap, $reimEmployee.getUserCode() + $threeBizCode, dateTime)) {
                amount = amount.add(allowanceAmount);
            }
            ...
            if ($travelPartnerInfo != null) {
                ...
                for(TravelPartner travelPartner : $travelPartnerInfo.getInternalTravelPartner) {
                    ...
                    logger.info("补贴参与人行程日期为" + dateTime + ",补贴参与人编号为" + travelPartner.getUserCode() + ",补贴标准金额为" + partnerAllowanceAmount + ",补贴标准编码为" + $threeBizCode);

                    /**判断日期是否重叠 */
                    if(!isDayOverlap($dateMap, travelPartner.getUserCode() + $threeBizCode, dateTime)) {
                        amount = amount.add(partnerAllowanceAmount);
                    }
                }
        ...
        if(amount.compareTo(BigDecimal.ZERO) == 1) {
            ...
            /**返回补贴结果 */
            AllowanceResult allowanceResult = new AllowanceResult($startDate, $endDate, $threeFeeCode, amount, $baseCcy, $collectionCcy);
            /**设置消费城市 */
            allowanceResult.setConsumeLocation($destination);
