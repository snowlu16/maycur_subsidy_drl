# real_2.16_含报销人和同行人_全天_按天汇总_两套标准
...
rule "初始化参数"
    ...
    when
        Reimburse(); // 判断是否有单据
    then
	String firstBizCode = "BT"; // 设置第一个标准补贴编码
		String firstFeeCode = "BT"; // 设置第一个费用类型编码
        String secondBizCode = "CLBT"; // 设置第二个标准补贴编码
        String secondFeeCode = "butie"; // 设置第二个费用类型编码
...
rule "报销人和参与人行程费用第一个补贴金额规则"
  ...
  // 判断补贴标准是否存在
        $map : Map(get($firstBizCode + "DESTINATION") != null) from $allowanceStandardInfo.getStandardMap();
    then
        /**设置补贴总金额 */
        ...
        if(amount.compareTo(BigDecimal.ZERO) == 1) {
              ...
              /**获取币种 */
            String currency = ((AllowanceStandard)$map.get($firstBizCode + "DESTINATION")).getCurrency();

            /**返回补贴结果 */
            AllowanceResult allowanceResult = new AllowanceResult($dateTime, $firstFeeCode, amount, currency, currency);
            /**设置消费城市 */
...
rule "报销人和参与人行程费用第二个补贴金额规则"
  ...
  // 判断补贴标准是否存在
        $map : Map(get($secondBizCode + "DESTINATION") != null) from $allowanceStandardInfo.getStandardMap();
    then
        /**设置补贴总金额 */
		BigDecimal amount = BigDecimal.ZERO;
        ...
        if($reimEmployee != null) {
            ...
            logger.info("补贴报销人行程日期为" + $dateTime + ",补贴报销人编号为" + $reimEmployee.getUserCode() + ",补贴标准金额为" + allowanceAmount + $secondBizCode);
        }
        ...
        if ($travelPartnerInfo != null) {
            ...
            for(TravelPartner travelPartner : $travelPartnerInfo.getInternalTravelPartner) {
                ...
                logger.info("补贴参与人行程日期为" + $dateTime + ",补贴参与人编号为" + travelPartner.getUserCode() + ",补贴标准金额为" + partnerAllowanceAmount + $secondBizCode);
        ...
        if(amount.compareTo(BigDecimal.ZERO) == 1) {
           ...
           /**获取币种 */
            String currency = ((AllowanceStandard)$map.get($secondBizCode + "DESTINATION")).getCurrency();

            /**返回补贴结果 */
            AllowanceResult allowanceResult = new AllowanceResult($dateTime, $secondFeeCode, amount, currency, currency);
            /**设置消费城市 */
            allowanceResult.setConsumeLocation($destination);
             insert(allowanceResult);
        }
