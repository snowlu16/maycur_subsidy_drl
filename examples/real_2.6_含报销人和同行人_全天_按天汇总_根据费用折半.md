# real_2.6_含报销人和同行人_全天_按天汇总_根据费用折半
...
rule "初始化参数"
    ...
    then
		...
		String feeCode="12345"; // 设置补贴费用类型编码
        String typeBizCode="2020_01"; // 设置需扣减费用类型编码
        boolean consumeDate=true; // 设置费用的消费时间类型 true-单个消费日期，false-时间段
        ...
        insert(new TravelHalfDayKeys(new HashMap(), dateList, bizCode, feeCode));
end
...
/**判乘以对应比例，若要扣减为0，则填写0 */
                amount = amount.add(allowanceAmount.multiply(0));
            } else {
...
rule "参与人行程费用规则"
    ...
    then
        ...
        if(!isDayOverlap($dateMap, $travelPartner.getUserCode(), $dateTime)) {
            ...
            if($dateList.contains($dateTime.toString("yyyy-MM-dd"))) {
	/**判乘以对应比例，若要扣减为0，则填写0 */
                amount = amount.add(partnerAllowanceAmount.multiply(0));
            } else {
